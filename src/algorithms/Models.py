...
class Img(Models):
    ...
    def definition(self,*args, **kwds):
        ...
        if(len(args) == 6 and isinstance(args[5], dict)):
            self.session = args[1]
            self.im = args[2]
            self.processed_images = args[3]
            self.probabilities = args[4]
            self.names = args[5]
            #deepexplainer
        elif(len(args) == 6 and isinstance(args[0], dict)):
            self.session = args[1]
            self.xs = args[2]
            self.labels = args[3]
            self.logits = args[4]
            self.processed_images = args[5]
            #lore
        elif(len(args) == 7): #and isinstance(args[1],
            self.inv_class_map = args[1]
            self.class_values = args[2]
    ...
    def setSplit(self,train_size = 0.9):
        # Split into test and training sets
        # Split at the given index
        split_index = int(train_size * self.n_images)
        shuffled_indices = np.random.permutation(self.n_images)
    ...
    def set_features(self,positive_only,num_features,hide_rest):
